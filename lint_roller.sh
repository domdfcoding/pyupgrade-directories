...
if [ -z "$(git status --porcelain --untracked-files=no)" ] || [ "$1" == "-f" ]; then
  ...
  autopep8 --in-place --select "$errors" -a --recursive pyupgrade_directories/
  autopep8 --in-place --select "$belligerent" -a -a -a -a -a --recursive pyupgrade_directories/


  echo "Running flake8"

    >&2 flake8 pyupgrade_directories/
